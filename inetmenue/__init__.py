...
class Parser:
    ...
    global_categories = {
        "https://cdn.inetmenue.de/media%2F00512471%2Fcdd3290ac929fba511b3d2f5497f4172560ece12.jpg":
        "*closed*",
        "*schneemann.gif":
        "*closed*",
        "https://cdn.inetmenue.de/media%2F00512471%2Fedc0f3199cbf3b7e3b570d7a6f27185bf088bdb0.jpg":
        "*ignore*",
        "https://cdn.inetmenue.de/media%2F00060380%2Fb0ab12b157398fdd8141d7eb71f4fc920c9925cf.jpg":
        "*ignore*",
        "https://cdn.inetmenue.de/media%2F00017709%2Fd92b8df29ef5be9a89f438f5f2af24189b85f4bc.png":
        "*ignore*",
        "https://cdn.inetmenue.de/media%2F00060385%2F9080_0000%2B-%2Blogo%2Bpartyk%25E3%25BCche%2Bkempe%2Bmit%2Bwww.jpg":
        "*ignore*",
        "https://cdn.inetmenue.de/media%2F00017709%2F683_peg-cat.jpg":
        "*ignore*",
        "https://cdn.inetmenue.de/media%2F00280005%2F04699e67275bb021b3a883e8f107a081452c0c2578b8bb819f2bf373e1710d20.jpg":
        "*ignore*",
        "https://cdn.inetmenue.de/media%2F00017709%2Fc6b053e7e3b5eca2162f8a7db28d2ef13d4a650b.jpg":
        "Nudelbuffet",
        "https://cdn.inetmenue.de/media%2F00209660%2F0d6cb6be82321a6db02c87fc126503ab9317a0e3.jpg":
        "DGE-Qualitätsstandard",
        "https://cdn.inetmenue.de/media%2F00143229%2F10597_mensa%2Bfps.jpg":
        "*ignore*",
        "https://cdn.inetmenue.de/media%2F00143229%2Fd7e19a1d1ad799edf4a25fdb9662ee0b06cfcff3.jpg":
        "Salat",
        "*_dummy%2Bklein.jpg":
        "*ignore*",
        "*_test-bild-gross.jpg":
        "*ignore*",
        "*_standardbild.jpg":
        "*ignore*",
    }
    ...
    def feed_all(self, ref: str, get_next_week=True) -> str:
        ...
        # This week
        url_parts = urllib.parse.urlsplit(self.canteens[ref]['source'],
                                          scheme="https")
    ...
    def parseMeals(self, ref: str, builder: pyopenmensa.feed.LazyBuilder,
                   html: str) -> str:
    ...
    def parseMealsSF(self, ref: str, builder: pyopenmensa.feed.LazyBuilder,
                     document: bs4.BeautifulSoup) -> str:
        ...
        for tr in document.select('.week_table tbody')[0].children:
            ...
            if "class" in tr.attrs and "menutime" in tr["class"]:
                ...
                for day_index, td in enumerate(tr.find_all("td")):
                    ...
                    builder.addMeal(dates[day_index], category_name,
                                    name.strip())

        return next_week_url

    def parseMealsFS(self, ref: str, builder: pyopenmensa.feed.LazyBuilder,
                     document: bs4.BeautifulSoup) -> str:
        ...
        for child in document.find(id='week-content').children:
            ...
            if child.name == "h2":
                ...
                for day_index, day_div in enumerate(menu_line.select('.day')):
                    ...
                    if header:
                    ...
                    elif day_div.select('.product .image') and day_div.select(
                            '.product .image')[0]['style']:
                        category_img = day_div.select('.product .image')[0][
                        ...
                        if category_img in predefined_categories:
                            ...
                            for query in predefined_categories:
                                if query.startswith(
                                        "$") and category_img.endswith(
                                            query[1:]):
                                    category_name = predefined_categories[
                                        query]
                                if query.startswith(
                                        "^") and category_img.startswith(
                                            query[1:]):
                                    category_name = predefined_categories[
                                        query]
                                if query.startswith(
                                        "*") and query[1:] in category_img:
                                    category_name = predefined_categories[
                                        query]
                            if not category_name:
                                logging.debug(
                    ...
                    ) or category_name == "*ignore*":
                        category_name = 'Essen %02d' % (category_index, )
                        logging.info(f"No category found, using default %r" %
                                     (category_name, ))
