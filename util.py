...
__all__ = [
    'xmlEscape', 'xmlRemoveInvalidChars', 'StyledLazyBuilder', 'nowBerlin',
    'weekdays_map'
]
...
class StyledLazyBuilder(LazyBuilder):
    ...
    def toXMLFeed(self, styles=defaultStyleSheets):
        ...
        if styles:
            for style in styles:
                xml_header += '<?xml-stylesheet href="' + xmlEscape(
                    style, True) + '" type="text/css"?>\n'
        return xmlRemoveInvalidChars(xml_header +
...
weekdays_map = [("Mo", "monday"), ("Tu", "tuesday"), ("We", "wednesday"),
